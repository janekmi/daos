...
#define CK_PRINT(ck, msg)                                                                          \
	do {                                                                                       \
		if (IS_CHECKER(ck)) {                                                              \
			(void)(ck)->ck_printf(ck, "%s" msg, (ck)->prefix);                         \
		}                                                                                  \
	} while (0)

#define CK_PRINTF(dp, fmt, ...)                                                                    \
	do {                                                                                       \
		if (IS_CHECKER(ck)) {                                                              \
			(void)(ck)->ck_printf(ck, "%s" fmt, (ck)->prefix, __VA_ARGS__);            \
		}                                                                                  \
	} while (0)
...
checker_print_indent_inc(struct checker *ck)
{
	...
	if (ck->level == CHECKER_INDENT_MAX) {
...
#define CK_INDENT(print, exp)                                                                      \
	do {                                                                                       \
		checker_print_indent_inc(dp);                                                      \
		exp;                                                                               \
		checker_print_indent_dec(dp);                                                      \
	} while (0)
