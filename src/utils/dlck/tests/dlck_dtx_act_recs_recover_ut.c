...
static const unsigned char      *Mltpl_co_uuids[]     = {Co_uuid_elem2.uuid, Co_uuid_elem1.uuid};
static const uint64_t            Mltpl_cont_handles[] = {CONT_HANDLE2, CONT_HANDLE1};

#define MLTPL_COUNT (ARRAY_SIZE(Mltpl_co_uuids))
...
{
	assert_int_equal(poh.cookie, POOL_HANDLE);
	assert_ptr_equal(co_uuid, mock_type(char *));

	int rc = mock_type(int);

	if (rc == DER_SUCCESS) {
		if (co_uuid == Ctrl.common.co_uuid || co_uuid == Co_uuid_elem1.uuid) {
			(*coh).cookie = CONT_HANDLE1;
		} else {
...
test_pool_open_fails(void **unused)
{
	...
	will_return(dlck_abt_pool_open, File1.po_uuid);
...
test_cont_recs_get_active_fails(void **unused)
{
	...
	will_return(__wrap_dlck_vos_cont_recs_get_active, CONT_HANDLE1);
	will_return(__wrap_dlck_vos_cont_recs_get_active, GENERIC_ERROR_RC);
	will_return(__wrap_vos_cont_close, CONT_HANDLE1);
...
test_dtx_act_recs_remove_fails(void **unused)
{
	...
	will_return(__wrap_dlck_vos_cont_recs_get_active, DER_SUCCESS);
	will_return(__wrap_d_calloc, 0);
	will_return(__wrap_dlck_dtx_act_recs_remove, CONT_HANDLE1);
...
test_dtx_act_recs_set_fails(void **unused)
{
	...
	will_return(__wrap_dlck_dtx_act_recs_set, CONT_HANDLE1);
...
static void
process_cont_will_return(const unsigned char *co_uuid, uint64_t cont_handle)
{
	will_return(__wrap_vos_cont_open, co_uuid);
	will_return(__wrap_vos_cont_open, DER_SUCCESS);
...
test_single_cont_success(void **unused)
{
	...
	will_return(dlck_abt_pool_open, File1.po_uuid);
	will_return(dlck_abt_pool_open, DER_SUCCESS);
	process_cont_will_return(Ctrl.common.co_uuid, CONT_HANDLE1);
	will_return(dlck_abt_pool_close, DER_SUCCESS);
	will_return(dlck_engine_xstream_fini, DER_SUCCESS);
	exec_one(&Xa);
	assert_int_equal(Xa.rc, DER_SUCCESS);
...
test_pool_cont_list_free_fails(void **unused)
{
	...
	for (int i = 0; i < MLTPL_COUNT; ++i) {
		process_cont_will_return(Mltpl_co_uuids[i], Mltpl_cont_handles[i]);
	}
...
static void
test_multiple_cont_success(void **unused)
{
	ctrl_default();
	...
	will_return(dlck_pool_cont_list_free, DER_SUCCESS);
	will_return(dlck_abt_pool_close, DER_SUCCESS);
	will_return(dlck_engine_xstream_fini, DER_SUCCESS);
	exec_one(&Xa);
	assert_int_equal(Xa.rc, DER_SUCCESS);
...
static void
test_multiple_pools_single_cont_success(void **unused)
{
	int rc;
	...
	/** set a single container */
	rc = uuid_parse(Co_uuid_str, Ctrl.common.co_uuid);
	assert_int_equal(rc, 0);

	will_return(dlck_engine_xstream_init, DER_SUCCESS);

	/** process the second pool */
	will_return(dlck_abt_pool_open, File2.po_uuid);
	will_return(dlck_abt_pool_open, DER_SUCCESS);
	process_cont_will_return(Ctrl.common.co_uuid, CONT_HANDLE1);
	will_return(dlck_abt_pool_close, DER_SUCCESS);

	/** process the first pool */
	will_return(dlck_abt_pool_open, File1.po_uuid);
	will_return(dlck_abt_pool_open, DER_SUCCESS);
	process_cont_will_return(Ctrl.common.co_uuid, CONT_HANDLE1);
	will_return(dlck_abt_pool_close, DER_SUCCESS);
...
static const struct CMUnitTest tests_all[] = {
    ...
    {"D121: multiple container success", test_multiple_cont_success, NULL, NULL},
    {"D122: multiple pools + a single container success", test_multiple_pools_single_cont_success,
     NULL, NULL},
};
...
main(int argc, char **argv)
{
	...
	assert_int_equal(ARRAY_SIZE(Mltpl_co_uuids), ARRAY_SIZE(Mltpl_cont_handles));
