...
static int
process_cont(daos_handle_t poh, uuid_t co_uuid, void *arg)
{
	struct dlck_print *dp = arg;
	daos_handle_t      coh;
	int                rc;

	rc = vos_cont_open(poh, co_uuid, dp, &coh);
	if (rc != DER_SUCCESS) {
		return rc;
	}
	...
	return vos_cont_close(coh);
}

static int
process_target(struct xstream_arg *xa, int idx)
{
	struct dlck_print *dp = &xa->ctrl->print;
	...
	d_list_for_each_entry(file, &xa->ctrl->files.list, link) {
		...
		if (uuid_is_null(xa->ctrl->common.co_uuid)) {
			rc = dlck_pool_foreach_cont(poh, process_cont, dp);
		} else {
			rc = process_cont(poh, xa->ctrl->common.co_uuid, dp);
		}
