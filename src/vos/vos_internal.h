 ...
 * \param[out]	dda	Array for storing the found active ilog entries.
 *
 * \retval 0		Success.
 * \retval -DER_*	Error when ilog_fetch fails.
 */
int
dlck_obj_get_active(daos_handle_t coh, struct vos_iterator *iter, struct dlck_dtx_rec_array *dda);
 ...
 */
int
dlck_irec_get_active(daos_handle_t coh, struct vos_iterator *iter, struct dlck_dtx_rec_array *dda);
 ...
 * \param[out]	dda	Array for storing the single value.
 *
