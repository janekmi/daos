...
static void
pmdk_log_function(enum pmemobj_log_level level, const char *file_name, const int line_no,
...
#define PMDK_LOG_NOCHECK(mask, fmt, ...)                                                           \
	d_log(mask, "%s:%d %s() " fmt, file_name, (unsigned)line_no, function_name, ##__VA_ARGS__)
