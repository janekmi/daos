...
/**
 * Invalidate contents of the provided buffer.
 *
 * Note: The buffer filled with 'x'-es is considered invalidated.
 */
static void
invalidate_buf(char *buf)
{
	memset(buf, 'x', BUF_SIZE - 1);
	buf[BUF_SIZE - 1] = '\0';
}
...
fetch(daos_handle_t chl, daos_unit_oid_t oid, daos_epoch_t epoch, daos_key_t *dkey, daos_iod_t *iod,
      d_sg_list_t *sgl)
{
	invalidate_buf(sgl->sg_iovs[0].iov_buf);
